...
function install_dcrd() {
  ...
  cp -f "/tmp/$TMPDIR/decred-linux-$MACHINE-v$VERSION/dcrd" $BINARYPATH && chown dcrd:dcrd $BINARYPATH &&chmod a+x $BINARYPATH
  cp -f "/tmp/$TMPDIR/dcrd.service" /etc/systemd/system/dcrd.service
